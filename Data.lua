...
if Whiten then
    ...
    if (paths.filep(Pfile) and paths.filep(invPfile) and paths.filep(meansfile)) then
        ...
        TrainData.data = unsup.zca_whiten(TrainData.data:double(), means, P, invP)
    else
        TrainData.data, means, P, invP = unsup.zca_whiten(TrainData.data:double())
        torch.save(Pfile,P)
        torch.save(invPfile,invP)
        torch.save(meansfile,means)
    end
    TestData.data = unsup.zca_whiten(TestData.data:double(), means, P, invP)
